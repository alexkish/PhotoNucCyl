...
#include "G4PhotoNuclearCrossSectionBe.hh"
...
//G4_DECLARE_XS_FACTORY(G4PhotoNuclearCrossSectionBe);


// Initialization of the statics
G4int G4PhotoNuclearCrossSectionBe::lastN=0;  
                            // The last N of calculated nucleus
G4int G4PhotoNuclearCrossSectionBe::lastZ=0;  
                            // The last Z of calculated nucleus
G4double G4PhotoNuclearCrossSectionBe::lastSig=0.; 
                            // Last value of the Cross Section
G4double* G4PhotoNuclearCrossSectionBe::lastGDR=0; 
                            // Pointer to the last array of GDR cross sections
G4double* G4PhotoNuclearCrossSectionBe::lastHEN=0; 
                            // Pointer to the last array of HEn cross sections
G4double G4PhotoNuclearCrossSectionBe::lastE=0.;  
                            // Last used in the cross section Energy
G4double G4PhotoNuclearCrossSectionBe::lastTH=0.; 
                            // Last value of the Energy Threshold (A-dependent)
G4double G4PhotoNuclearCrossSectionBe::lastSP=0.; 
                            // Last value of the ShadowingPomeron (A-dependent)


// Vector of pointers to the GDRPhotonuclearCrossSection
std::vector<G4double*> G4PhotoNuclearCrossSectionBe::GDR;

// Vector of pointers to the HighEnPhotonuclearCrossSect
std::vector<G4double*> G4PhotoNuclearCrossSectionBe::HEN;


G4PhotoNuclearCrossSectionBe::G4PhotoNuclearCrossSectionBe(const G4String& name)
 : G4VCrossSectionDataSet(name)
{
  SetupBeCrossSections();
}

G4PhotoNuclearCrossSectionBe::~G4PhotoNuclearCrossSectionBe()
{
...
void
G4PhotoNuclearCrossSectionBe::CrossSectionDescription(std::ostream& outFile) const
{
  outFile << "G4PhotoNuclearCrossSectionBe provides the total inelastic\n"
...
G4bool
G4PhotoNuclearCrossSectionBe::IsIsoApplicable(const G4DynamicParticle* particle, 
					    G4int /*Z*/, G4int /*A*/,
					    const G4Element*, const G4Material*)
{
  ...
  //G4cout <<"IsIsoApplicable? " << result << G4endl;
  return result;
}
...
// (E in GeV, CS in mb)
G4double 
G4PhotoNuclearCrossSectionBe::GetIsoCrossSection(const G4DynamicParticle* aPart,
                                               G4int ZZ, G4int AA, const G4Isotope*,
					       const G4Element*, const G4Material*)
{
 
  const G4double Energy = aPart->GetKineticEnergy()/CLHEP::MeV;
  //Special case in Be
  if (ZZ==4 && AA==9) return GetBeCrossSection(Energy);
...
#ifdef debug
  G4cout << "G4PhotoNuclearCrossSectionBe::GetCS:N=" << targN << ",Z=" 
         << targZ << ",E=" << Energy << G4endl;
#endif
  if (Energy<THmin) {
	//G4cout << "returned zero at THmin" << G4endl;
	return 0.;
  }
  G4double sigma=0.;
  if( aPart->GetDefinition()->GetPDGEncoding() == 22 )
  {
    ...
    if (Energy<lastTH)
    {
      lastE=Energy;
      lastSig=0.;
	  //G4cout << "Returned zero at magic" << G4endl;
      return 0.;
    }
	...
	//G4cout <<"Returning zero at end " << G4endl;
	return 0.;
  }
  if(sigma<0.) return 0.;

  //G4cout <<"Returning at the end: " << sigma*millibarn << G4endl;
  return sigma*CLHEP::millibarn;
}

// Gives the threshold energy for different nuclei (min of p- and n-threshold)
G4double G4PhotoNuclearCrossSectionBe::ThresholdEnergy(G4int Z, G4int N)
{
  // CHIPS - Direct GEANT
  ...
  {
    G4cerr << "G4PhotoNuclearCrossSectionBe::ThrEn:Z=" << Z-1 << ",A=" 
  ...
  */
  ...
  {
    G4cerr<<"G4PhotoNuclearCrossSectionBe::ThreshEn:Z="<<Z<<",A="<<A-1
...
//  to X point // @@ !!! Not used !!!
//  G4double G4PhotoNuclearCrossSectionBe::
...
//
G4double 
G4PhotoNuclearCrossSectionBe::EquLinearFit(G4double X, G4int N, 
                                         const G4double X0, const G4double DX, 
                                         const G4double* Y)
{
  if(DX<=0. || N<2)
  {
    G4cout<<"***G4PhotoNuclearCrossSectionBe::EquLinearFit: DX="<<DX<<", N="<<N<<G4endl;
    return Y[0];
  }
  G4int    N2=N-2;
  G4double d=(X-X0)/DX;
...
G4int 
G4PhotoNuclearCrossSectionBe::GetFunctions(G4double a, G4double* y, G4double* z)
{
...
void G4PhotoNuclearCrossSectionBe::SetupBeCrossSections() 
{
  //Energies are in ev!
...
/*  // debug
  fBeEnerg[i] = 1000000.0; fBeCross[i++] = 0.0;
  fBeEnerg[i] = 1000000.0; fBeCross[i++] = 0.0;
  fBeEnerg[i] = 1573100.0; fBeCross[i++] = 0.0;
  fBeEnerg[i] = 1670000.0; fBeCross[i++] = 1.0;
  fBeEnerg[i] = 1680000.0; fBeCross[i++] = 1.0;
  fBeEnerg[i] = 1690000.0; fBeCross[i++] = 1.0;
  fBeEnerg[i] = 1700000.0; fBeCross[i++] = 1.0;
  fBeEnerg[i] = 1710000.0; fBeCross[i++] = 0.1;
  fBeEnerg[i] = 1720000.0; fBeCross[i++] = 0.1;
  fBeEnerg[i] = 1730000.0; fBeCross[i++] = 0.1;
  fBeEnerg[i] = 1740000.0; fBeCross[i++] = 0.1;
  fBeEnerg[i] = 1750000.0; fBeCross[i++] = 0.1;
*/
/*  // modified
  fBeEnerg[i] = 1000000.0; fBeCross[i++] = 0.0;
  fBeEnerg[i] = 1000000.0; fBeCross[i++] = 0.0;
  fBeEnerg[i] = 1573100.0; fBeCross[i++] = 0.0;
  fBeEnerg[i] = 1670000.0; fBeCross[i++] = 3.06824E-4;
  fBeEnerg[i] = 1680000.0; fBeCross[i++] = 0.0011196;
  fBeEnerg[i] = 1690000.0; fBeCross[i++] = 0.0014134;
  fBeEnerg[i] = 1700000.0; fBeCross[i++] = 0.001414;
  fBeEnerg[i] = 1710000.0; fBeCross[i++] = 0.0013344;
  fBeEnerg[i] = 1720000.0; fBeCross[i++] = 0.0012431;
  fBeEnerg[i] = 1730000.0; fBeCross[i++] = 0.0011582;
  fBeEnerg[i] = 1740000.0; fBeCross[i++] = 0.0010834;
  fBeEnerg[i] = 1750000.0; fBeCross[i++] = 0.0010183;
*/  // real part
  fBeEnerg[i] = 1000000.0; fBeCross[i++] = 0.0;
  fBeEnerg[i] = 1000000.0; fBeCross[i++] = 1.1393E-8;
  fBeEnerg[i] = 1573100.0; fBeCross[i++] = 4.86369E-9;
  fBeEnerg[i] = 1670000.0; fBeCross[i++] = 3.06824E-4;
  fBeEnerg[i] = 1680000.0; fBeCross[i++] = 0.0011196;
  fBeEnerg[i] = 1690000.0; fBeCross[i++] = 0.0014134;
  fBeEnerg[i] = 1700000.0; fBeCross[i++] = 0.001414;
  ...
  fBeEnerg[i] = 1760000.0; fBeCross[i++] = 9.61523E-4;
  fBeEnerg[i] = 1770000.0; fBeCross[i++] = 9.11833E-4;
  fBeEnerg[i] = 1780000.0; fBeCross[i++] = 8.68013E-4;
  fBeEnerg[i] = 1790000.0; fBeCross[i++] = 8.29102E-4;
  fBeEnerg[i] = 1800000.0; fBeCross[i++] = 7.94322E-4;
  fBeEnerg[i] = 1810000.0; fBeCross[i++] = 7.63022E-4;
  fBeEnerg[i] = 1820000.0; fBeCross[i++] = 7.34712E-4;
  fBeEnerg[i] = 1830000.0; fBeCross[i++] = 7.08962E-4;
  fBeEnerg[i] = 1840000.0; fBeCross[i++] = 6.85442E-4;
  fBeEnerg[i] = 1850000.0; fBeCross[i++] = 6.63872E-4;
  fBeEnerg[i] = 1860000.0; fBeCross[i++] = 6.44012E-4;
  fBeEnerg[i] = 1880000.0; fBeCross[i++] = 6.08081E-4;
  fBeEnerg[i] = 1900000.0; fBeCross[i++] = 5.76771E-4;
  fBeEnerg[i] = 1920000.0; fBeCross[i++] = 5.49361E-4;
  fBeEnerg[i] = 1940000.0; fBeCross[i++] = 5.25201E-4;
  fBeEnerg[i] = 1960000.0; fBeCross[i++] = 5.0378E-4;
  fBeEnerg[i] = 1980000.0; fBeCross[i++] = 4.8473E-4;
  fBeEnerg[i] = 2000000.0; fBeCross[i++] = 4.6774E-4;
  fBeEnerg[i] = 2030000.0; fBeCross[i++] = 4.4567E-4;
  fBeEnerg[i] = 2060000.0; fBeCross[i++] = 4.2727E-4;
  fBeEnerg[i] = 2090000.0; fBeCross[i++] = 4.1193E-4;
  fBeEnerg[i] = 2120000.0; fBeCross[i++] = 3.998E-4;
  fBeEnerg[i] = 2140000.0; fBeCross[i++] = 3.9361E-4;
  fBeEnerg[i] = 2160000.0; fBeCross[i++] = 3.8908E-4;
  fBeEnerg[i] = 2180000.0; fBeCross[i++] = 3.8645E-4;
  fBeEnerg[i] = 2190000.0; fBeCross[i++] = 3.8596E-4;
  fBeEnerg[i] = 2210000.0; fBeCross[i++] = 3.8695E-4;
  fBeEnerg[i] = 2220000.0; fBeCross[i++] = 3.886E-4;
  fBeEnerg[i] = 2230000.0; fBeCross[i++] = 3.9117E-4;
  fBeEnerg[i] = 2240000.0; fBeCross[i++] = 3.9483E-4;
  fBeEnerg[i] = 2250000.0; fBeCross[i++] = 3.9976E-4;
  fBeEnerg[i] = 2260000.0; fBeCross[i++] = 4.062E-4;
  fBeEnerg[i] = 2270000.0; fBeCross[i++] = 4.1438E-4;
  fBeEnerg[i] = 2280000.0; fBeCross[i++] = 4.2468E-4;
  fBeEnerg[i] = 2290000.0; fBeCross[i++] = 4.3769E-4;
  fBeEnerg[i] = 2300000.0; fBeCross[i++] = 4.5409E-4;
  fBeEnerg[i] = 2310000.0; fBeCross[i++] = 4.7478E-4;
  fBeEnerg[i] = 2320000.0; fBeCross[i++] = 5.0094E-4;
  fBeEnerg[i] = 2330000.0; fBeCross[i++] = 5.3418E-4;
  fBeEnerg[i] = 2340000.0; fBeCross[i++] = 5.7662E-4;
  fBeEnerg[i] = 2350000.0; fBeCross[i++] = 6.3105E-4;
  fBeEnerg[i] = 2360000.0; fBeCross[i++] = 7.0107E-4;
  fBeEnerg[i] = 2370000.0; fBeCross[i++] = 7.9093E-4;
  fBeEnerg[i] = 2380000.0; fBeCross[i++] = 9.0478E-4;
  fBeEnerg[i] = 2390000.0; fBeCross[i++] = 0.0010445;
  fBeEnerg[i] = 2410000.0; fBeCross[i++] = 0.0013661;
  fBeEnerg[i] = 2420000.0; fBeCross[i++] = 0.0014908;
  fBeEnerg[i] = 2430000.0; fBeCross[i++] = 0.0015368;
  fBeEnerg[i] = 2440000.0; fBeCross[i++] = 0.0014874;
  fBeEnerg[i] = 2450000.0; fBeCross[i++] = 0.0013651;
  fBeEnerg[i] = 2470000.0; fBeCross[i++] = 0.0010604;
  fBeEnerg[i] = 2480000.0; fBeCross[i++] = 9.2792E-4;
  fBeEnerg[i] = 2490000.0; fBeCross[i++] = 8.1895E-4;
  fBeEnerg[i] = 2500000.0; fBeCross[i++] = 7.3166E-4;
  fBeEnerg[i] = 2510000.0; fBeCross[i++] = 6.6245E-4;
  fBeEnerg[i] = 2520000.0; fBeCross[i++] = 6.0761E-4;
  fBeEnerg[i] = 2530000.0; fBeCross[i++] = 5.6402E-4;
  fBeEnerg[i] = 2540000.0; fBeCross[i++] = 5.292E-4;
  fBeEnerg[i] = 2550000.0; fBeCross[i++] = 5.0125E-4;
  fBeEnerg[i] = 2560000.0; fBeCross[i++] = 4.7873E-4;
  fBeEnerg[i] = 2570000.0; fBeCross[i++] = 4.6056E-4;
  fBeEnerg[i] = 2580000.0; fBeCross[i++] = 4.4591E-4;
  fBeEnerg[i] = 2590000.0; fBeCross[i++] = 4.3417E-4;
  fBeEnerg[i] = 2600000.0; fBeCross[i++] = 4.2485E-4;
  fBeEnerg[i] = 2650000.0; fBeCross[i++] = 4.0451E-4;
  fBeEnerg[i] = 2700000.0; fBeCross[i++] = 4.1467E-4;
  fBeEnerg[i] = 2750000.0; fBeCross[i++] = 4.5231E-4;
  fBeEnerg[i] = 2800000.0; fBeCross[i++] = 5.2479E-4;
  fBeEnerg[i] = 2850000.0; fBeCross[i++] = 6.4859E-4;
  fBeEnerg[i] = 2900000.0; fBeCross[i++] = 8.4856E-4;
  fBeEnerg[i] = 2950000.0; fBeCross[i++] = 0.0011366;
  fBeEnerg[i] = 3000000.0; fBeCross[i++] = 0.0014292;
  fBeEnerg[i] = 3050000.0; fBeCross[i++] = 0.0015033;
  fBeEnerg[i] = 3100000.0; fBeCross[i++] = 0.0012954;
  fBeEnerg[i] = 3150000.0; fBeCross[i++] = 0.0010089;
  fBeEnerg[i] = 3200000.0; fBeCross[i++] = 7.7892E-4;
  fBeEnerg[i] = 3250000.0; fBeCross[i++] = 6.2022E-4;
  fBeEnerg[i] = 3300000.0; fBeCross[i++] = 5.1364E-4;
  fBeEnerg[i] = 3350000.0; fBeCross[i++] = 4.4115E-4;
  fBeEnerg[i] = 3400000.0; fBeCross[i++] = 3.9074E-4;
  fBeEnerg[i] = 3450000.0; fBeCross[i++] = 3.5494E-4;
  fBeEnerg[i] = 3500000.0; fBeCross[i++] = 3.2914E-4;
  fBeEnerg[i] = 4000000.0; fBeCross[i++] = 2.8274E-4;
  fBeEnerg[i] = 4500000.0; fBeCross[i++] = 4.3535E-4;
  fBeEnerg[i] = 5000000.0; fBeCross[i++] = 9.1038E-4;
  fBeEnerg[i] = 5500000.0; fBeCross[i++] = 9.8003E-4;
  fBeEnerg[i] = 6000000.0; fBeCross[i++] = 0.0010303;
  fBeEnerg[i] = 6500000.0; fBeCross[i++] = 0.0012719;
  fBeEnerg[i] = 7000000.0; fBeCross[i++] = 0.0013181;
  fBeEnerg[i] = 7500000.0; fBeCross[i++] = 0.0013061;
  fBeEnerg[i] = 8500000.0; fBeCross[i++] = 0.0013993;
  fBeEnerg[i] = 9000000.0; fBeCross[i++] = 0.0013875;
  fBeEnerg[i] = 9500000.0; fBeCross[i++] = 0.0013078;
  fBeEnerg[i] = 1.0E7;     fBeCross[i++] = 0.0011843;
  fBeEnerg[i] = 1.05E7;    fBeCross[i++] = 0.0010508;
  fBeEnerg[i] = 1.1E7;     fBeCross[i++] = 9.2812E-4;
  fBeEnerg[i] = 1.15E7;    fBeCross[i++] = 8.2393E-4;
  fBeEnerg[i] = 1.2E7;     fBeCross[i++] = 7.3798E-4;
  fBeEnerg[i] = 1.25E7;    fBeCross[i++] = 6.6982E-4;
  fBeEnerg[i] = 1.3E7;     fBeCross[i++] = 6.1799E-4;
  fBeEnerg[i] = 1.35E7;    fBeCross[i++] = 5.8118E-4;
  fBeEnerg[i] = 1.4E7;     fBeCross[i++] = 5.6585E-4;
  fBeEnerg[i] = 1.45E7;    fBeCross[i++] = 6.551E-4;
  fBeEnerg[i] = 1.5E7;     fBeCross[i++] = 5.3918E-4;
  fBeEnerg[i] = 1.55E7;    fBeCross[i++] = 5.4128E-4;
  fBeEnerg[i] = 1.57E7;    fBeCross[i++] = 5.5239E-4;
  fBeEnerg[i] = 1.6E7;     fBeCross[i++] = 5.5834E-4;
  fBeEnerg[i] = 1.65E7;    fBeCross[i++] = 6.82401E-4;
  fBeEnerg[i] = 1.6712E7;  fBeCross[i++] = 7.46493E-4;
  fBeEnerg[i] = 1.7E7;     fBeCross[i++] = 8.36907E-4;
  fBeEnerg[i] = 1.75E7;    fBeCross[i++] = 0.00103135;
  fBeEnerg[i] = 1.7707E7;  fBeCross[i++] = 0.00112971;
  fBeEnerg[i] = 1.8E7;     fBeCross[i++] = 0.00127893;
  fBeEnerg[i] = 1.85E7;    fBeCross[i++] = 0.00159787;
  fBeEnerg[i] = 1.9E7;     fBeCross[i++] = 0.00201139;
  fBeEnerg[i] = 1.95E7;    fBeCross[i++] = 0.00254242;
  fBeEnerg[i] = 2.0E7;     fBeCross[i++] = 0.00319375;
  fBeEnerg[i] = 2.0E7;     fBeCross[i++] = 0.003122;
  fBeEnerg[i] = 2.0588E7;  fBeCross[i++] = 0.00376146;
  fBeEnerg[i] = 2.1E7;     fBeCross[i++] = 0.00428269;
  fBeEnerg[i] = 2.2E7;     fBeCross[i++] = 0.00499254;
  fBeEnerg[i] = 2.3E7;     fBeCross[i++] = 0.00508541;
  fBeEnerg[i] = 2.4E7;     fBeCross[i++] = 0.00526455;
  fBeEnerg[i] = 2.5E7;     fBeCross[i++] = 0.00526812;
  fBeEnerg[i] = 2.6E7;     fBeCross[i++] = 0.00529323;
  fBeEnerg[i] = 2.7E7;     fBeCross[i++] = 0.00525456;
  fBeEnerg[i] = 2.8E7;     fBeCross[i++] = 0.00514868;
  fBeEnerg[i] = 2.9E7;     fBeCross[i++] = 0.00498366;
  fBeEnerg[i] = 3.0E7;     fBeCross[i++] = 0.00477201;
  G4cout << "SETUP SPECIAL BE CROSS-SECTIONS" << G4endl;
}

G4double G4PhotoNuclearCrossSectionBe::GetBeCrossSection(G4double energy_in_mev)
{
  G4double energy_in_ev = energy_in_mev * 1.e6;

  int k = 0; 
  for (k=0; k<141; ++k) if (energy_in_ev<fBeEnerg[k]) break;
  G4double interp(0);
  if (k>0 && k<141) {
	G4double en_diff_low = energy_in_ev - fBeEnerg[k-1];
	G4double en_diff_high = fBeEnerg[k] - energy_in_ev;
	interp = (en_diff_low * fBeCross[k] + en_diff_high * fBeCross[k-1]) / (en_diff_low + en_diff_high);
/*	G4cout <<">>> k             = "<< k 				<< G4endl;
	G4cout <<">>> fBeEnerg[k]   = "<< fBeEnerg[k] 	<< G4endl;
	G4cout <<">>> fBeEnerg[k-1] = "<< fBeEnerg[k-1] << G4endl;
	G4cout <<">>> energy_in_ev  = "<< energy_in_ev 	<< G4endl;
	G4cout <<">>> en_diff_low   = "<< en_diff_low 	<< G4endl;
	G4cout <<">>> en_diff_high  = "<< en_diff_high 	<< G4endl;
	G4cout <<">>> interp        = "<< interp 		<< G4endl;
*/  }
  else {
	if (k==141) --k;
	interp = fBeCross[k];
